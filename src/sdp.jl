function certify_Ising_gap(N::Int, H::ncpoly, gamma, d::Int; QUIET=false)
    ...
    lb = length.(basis)
    gbasis = [get_wbasis(N, d-1, label=i) for i in [2]] 
    lgb = length.(gbasis)
    bs = [lb; lgb]
    # if d > 2
...
function certify_Heisenberg_kagome_gap(N::Int, H::ncpoly, triples, edges, gamma, d::Int; QUIET=false)
    println("********************************** SpectralGap **********************************")
    println("SpectralGap is launching...")
    basis = [get_kagome_basis(N, triples, edges, d, label=i) for i = 1:2]
    lb = length.(basis)
    gbasis = [get_kagome_wbasis(N, d-1, label=i) for i = 2:2] 
    lgb = length.(gbasis)
    bs = [lb; lgb]
    if QUIET == false
        println("The block sizes are $bs.")
    ...
    for l = 1:length(gbasis)
        ...
        for i = 1:lgb[l], j = i:lgb[l]
            ...
            if !isz(gbasis[l][i]) && !isz(gbasis[l][j])
                Locb = bfind(tsupp, sort([reduce4(gbasis[l][i], N), reduce4(gbasis[l][j], N)]))
                if i == j
                    println(sort([reduce4(gbasis[l][i], N), reduce4(gbasis[l][j], N)]))
                    @inbounds add_to_expression!(cons[Locb], gamma, gpos[l][i, j])
                else
                    @inbounds add_to_expression!(cons[Locb], 2*gamma, gpos[l][i, j])
                end
